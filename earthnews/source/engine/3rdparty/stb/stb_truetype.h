...
#define STB_TRUETYPE_IMPLEMENTATION
   ...
   #ifndef stbtt_uint8
   typedef cxu8   stbtt_uint8;
   typedef cxi8   stbtt_int8;
   typedef cxu16  stbtt_uint16;
   typedef cxi16  stbtt_int16;
   typedef cxu32  stbtt_uint32;
   typedef cxi32  stbtt_int32;
   #endif
...
//#include <malloc.h>
   ...
   #ifndef STBTT_assert
//#include <assert.h>
   #define STBTT_assert(x)    CX_ASSERT(x)
   #endif
...
extern int stbtt_BakeFontBitmap2(const unsigned char *data, int offset,  // font location (use offset=0 for plain .ttf)
                                float pixel_height,                     // height of font in pixels
                                unsigned char *pixels, int pw, int ph,  // bitmap to be filled in
                                unsigned int *unicode_codepts, int num_codepts,          // characters to bake
                                stbtt_bakedchar *chardata)
{
  ...
  for (i=0; i < num_codepts; ++i)
  {
    int code_pt = unicode_codepts [i];
    int advance, lsb, x0,y0,x1,y1,gw,gh;
    int g = stbtt_FindGlyphIndex(&f, code_pt);
